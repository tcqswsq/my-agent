...
class HybridRetriever:
    ...
    def _tokenize(self, text: str) -> list:
        import re
        return re.findall(r"[a-zA-Z0-9]+", text.lower()) + re.findall(r"[\u4e00-\u9fff]", text)

    def _build_bm25(self):
        cache = Path("storage/bm25_cache.json")
        if cache.exists():
            data = json.loads(cache.read_text())
            self._bm25_ids = data["ids"]
        else:
            data = {"ids": [], "texts": []}
            p = Path("storage/clean_texts.jsonl")
            if p.exists():
                for line in p.read_text(encoding="utf-8").splitlines():
                    if not line.strip(): continue
                    obj = json.loads(line)
                    data["ids"].append(obj["chunk_id"])
                    data["texts"].append(obj["text"])
            cache.write_text(json.dumps(data, ensure_ascii=False))
            self._bm25_ids = data["ids"]
        if self._bm25_ids:
            texts = json.loads(cache.read_text())["texts"]
    ...
    def retrieve(self, query: str, top_k=50, top_n=10, filter_file_ids=None, filter_levels=None, alpha=0.7):
        ...
        if q_emb.ndim == 2:
            q_emb = q_emb[0]
        q_emb = q_emb.tolist()
        r = self.vs.query(q_emb, top_k=top_k * 2,
        ...
        v_scores = {cid: max(0.0, 1.0 - d) for cid, d in zip(r["ids"][0], r["distances"][0])} if r.get(
            "distances") else {}
        b_scores = self._bm25_search(query, top_k=top_k * 2)
        ...
        def norm(s):
            return {k: (v - min(s.values())) / (max(s.values()) - min(s.values())) if max(s.values()) > min(
                s.values()) else 0.5 for k, v in s.items()}

        fused = {cid: alpha * norm(v_scores).get(cid, 0) + (1 - alpha) * norm(b_scores).get(cid, 0) for cid in
                 candidates}
